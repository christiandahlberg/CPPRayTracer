		...
		for (int b = -11; b < 11; b++) {
			...
			if ((center - vec3(4, 0.2, 0)).length() > 0.9) {
				if (choose_mat < 0.8) {  // diffuse
					list[i++] = new sphere(center, 0.2, new lambertian(vec3(((double)rand() / (RAND_MAX))*((double)rand() / (RAND_MAX)),
						((double)rand() / (RAND_MAX))*((double)rand() / (RAND_MAX)),
						((double)rand() / (RAND_MAX))*((double)rand() / (RAND_MAX)))));
				}
				else if (choose_mat < 0.95) { // metal
					list[i++] = new sphere(center, 0.2,
						new metal(vec3(0.5*(1 + ((double)rand() / (RAND_MAX))), 0.5*(1 + ((double)rand() / (RAND_MAX))), 
							0.5*(1 + ((double)rand() / (RAND_MAX)))), 0.5*((double)rand() / (RAND_MAX))));
				}
				else {  // glass
